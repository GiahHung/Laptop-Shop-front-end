...
class Printer extends Component {
  ...
  componentDidMount() {
    this.props.fetchPrinter();
  }

  componentDidUpdate(prevProps, prevState, snapSho) {
    if (prevProps.arrPrinter !== this.props.arrPrinter) {
      this.setState({
        arrProduct: this.props.arrPrinter,
      });
    }
  }
...
const mapStateToProps = (state) => {
  return {
    arrPrinter: state.admin.arrPrinter,
...
const mapDispatchToProps = (dispatch) => {
  return {
    fetchPrinter: () => dispatch(actions.fetchPrinter()),
    addProductToCart: (data, userId) =>
      dispatch(actions.addProductToCart(data, userId)),
  };
};
function FunctionPrinter(props) {
  const navigate = useNavigate();
  return <Printer {...props} navigate={navigate} />;
}
export default connect(mapStateToProps, mapDispatchToProps)(FunctionPrinter);
