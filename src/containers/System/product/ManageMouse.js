...
class ManageMouse extends Component {
  constructor(props) {
    super(props);
    this.state = {
      arrMouseCategory: [],
  ...
  componentDidMount() {
    ...
    this.props.fetchMouseBrand();
    this.props.fetchStatusProductId();
    this.props.fetchMouseCategory();
  }
  ...
  componentDidUpdate(prevProps, prevState, snapShot) {
    ...
    if (prevProps.mouseCategoryIds !== this.props.mouseCategoryIds) {
      let arrMouseCategory = this.props.mouseCategoryIds;
      this.setState({
        arrMouseCategory: arrMouseCategory,
        mouseCategoryId: arrMouseCategory && arrMouseCategory.length > 0 ? arrMouseCategory[0].keyMap : "",
      });
    }
      ...
      this.props.createProduct(
        {
          ...
          mouseCategory: this.state.mouseCategoryId,
  ...
  handleEditProduct = (product) => {
    ...
    this.setState({
      ...
      mouseCategoryId: product.mouseCategory,
  ...
  render() {
    let arrMouseCategory = this.state.arrMouseCategory;
    let arrStatus = this.state.arrStatus;
    let arrBrand = this.state.arrBrand;
    let arrProduct = this.state.arrProduct;
    let { nameProduct, price, discount, mouseCategoryId, brandId, statusId, action } =
      this.state;
    console.log("check data", this.props.brandIds);
    return (
      <React.Fragment>
        <div className="title">PHỤ KIỆN MÁY TÍNH</div>
        <div className="frm-input container">
          <div className="row">
            ...
            <div className="col-4 form-group mt-3">
              ...
              <select
                className="form-control"
                value={mouseCategoryId}
                onChange={(event) => {
                  this.handleOnchangeInput(event, "mouseCategoryId");
                }}
              >
                {arrMouseCategory &&
                  arrMouseCategory.length > 0 &&
                  arrMouseCategory.map((item, index) => {
                    return (
                      <option key={index} value={item.keyMap}>
                        {item.value}
                      </option>
...
const mapStateToProps = (state) => {
  return {
    mouseCategoryIds: state.admin.mouseCategoryIds,
    statusProducts: state.admin.statusProducts,
    brandIds: state.admin.mouseBrandIds,
...
const mapDispatchToProps = (dispatch) => {
  return {
    fetchMouseCategory: () => dispatch(actions.fetchMouseCategory()),
    fetchMouseBrand: () => dispatch(actions.fetchMouseBrand()),
...
export default connect(mapStateToProps, mapDispatchToProps)(ManageMouse);
